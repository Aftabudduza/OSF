...
public partial class Admin_User : System.Web.UI.Page
{
    ...
    protected void lbtnView_Click(object sender, System.EventArgs e)
    {
        ...
        if (nId > 0)
        {
            ImageButton btndetails = sender as ImageButton;
            Users uObj = new Users(cmscon.CONNECTIONSTRING);
            DataTable dt = cmscon.getRows(string.Format("SELECT u.*, Job = (SELECT Name From BasicData WHERE BasicDataID=u.JobID), Department = (SELECT Name From BasicData WHERE BasicDataID=u.DepartmentID), Location = (SELECT Name From BasicData WHERE BasicDataID=u.LocationID) FROM users u WHERE u.UserID=1SELECT u.*, Job = (SELECT Name From BasicData WHERE BasicDataID=u.JobID), Department = (SELECT Name From BasicData WHERE BasicDataID=u.DepartmentID), Location = (SELECT Name From BasicData WHERE BasicDataID=u.LocationID) FROM users u WHERE u.UserID={0}", Convert.ToInt32(vId.Value)));
            if (dt != null)
            {
    ...
    private void FillControls(DataTable dt)
    {
        ...
        {
            imgUserView.Attributes["src"] = "http://localhost:5446/OSF/Images/Users/" + dt.Rows[0]["Picture"].ToString();
